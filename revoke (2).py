...
def save_config(api_id, api_hash, group, public_group="no"):
    config = {
        "api_id": api_id,
        "api_hash": api_hash,
        "group": group,
        "public_group": public_group.lower()
    }
...
async def start_monitoring(client):
    ...
    async def on_message_deleted(event):
        check_and_clean_cache()
        config = load_config()
        public_group_enabled = config.get("public_group", "no") == "yes"
        ...
        for msg_id in event.deleted_ids:
            if event.chat_id:
                try:
                    ...
                    if is_public and not public_group_enabled:
                        return
...
def main():
    ...
    if not config:
        ...
        group = input("Enter your Telegram group link (deleted message comes here): ")
        public_group = input("Monitor public groups/channels? (yes/no): ")
        save_config(api_id, api_hash, group, public_group)
    else:
